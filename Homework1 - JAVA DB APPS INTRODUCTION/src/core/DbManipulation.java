...
public class DbManipulation {
    private final String[] INSERT_DATA = {
            ...
            "ALTER TABLE minions_villains\n" +
                    "ADD CONSTRAINT fk_minions_villains\n" +
    ...
    private static final String[] TRUNCATE_DB = {
            "alter table minions_villains drop constraint fk_minions_villains;",
            "alter table minions_villains drop constraint fk_villains_minions;",
            "alter table minions drop constraint fk_minions_towns;",
            "truncate table minions_villains;",
            "truncate table minions;",
            "truncate table villains;",
            "truncate table towns;"
    };

    public void insertData(Connection connection) throws SQLException {
        Statement preparedStatement = connection.createStatement();
        for (String insert_datum : INSERT_DATA) {
            preparedStatement.addBatch(insert_datum);
        }
    ...
    public void truncateData(Connection connection) throws SQLException {
        Statement preparedStatement = connection.createStatement();
        for (String insert_datum : TRUNCATE_DB) {
            preparedStatement.addBatch(insert_datum);
        }
        preparedStatement.executeBatch();
        preparedStatement.close();
